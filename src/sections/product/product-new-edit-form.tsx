...
//     // chỉ áp dụng khi tạo mới
//     if (currentProduct) return;

//     methods.setValue(
//       "vat",
//       defaultSetting.productVat
//     );

//     methods.setValue(
//       "unitId",
//       defaultSetting.productUnitId
//     );

//     methods.setValue(
//       "manufacturer",
//       defaultSetting.defaultSupplierName
//     );
//   }, [defaultSetting, currentProduct]);
...
//             if (res.data) {
//               setDefaultSetting({
//                 productVat: res.data.productVat ?? 0,
//                 productUnitId: res.data.productUnitId ?? 0,
//                 defaultSupplierName:
//                   res.data.defaultSupplierName ?? "",
//               });
//             }
//           } catch (error) {
//             console.error(error);
...
export function ProductNewEditForm({ open, onClose, selectedId }: Props) {
  ...
  // 1. Fetch dữ liệu sản phẩm hiện tại nếu đang ở chế độ Edit
  const { product: currentProduct, productLoading } = useGetProduct(selectedId, {
    enabled: !!selectedId,
  });

  // 2. Fetch danh mục và đơn vị tính
  const { categories, categoriesLoading } = useGetCategories({
    pageNumber: 1,
    pageSize: 999,
    key: debouncedCategoryKw,
  ...
  const defaultValues: NewProductSchemaType = {
    ...
    vat: 0,
    image: null,
    Folder: 'SanPham', // Chuyển mặc định sang thư mục SanPham cho đúng ngữ cảnh
  };
  ...
  // 4. EFFECT: Gọi API lấy cấu hình mặc định (Chỉ chạy khi mở form tạo mới)
  useEffect(() => {
    const fetchDefaultSetting = async () => {
      try {
        ...
        if (res.data) {
          setDefaultSetting({
            productVat: res.data.productVat ?? 0,
            productUnitId: res.data.productUnitId ?? 0,
            defaultSupplierName: res.data.defaultSupplierName ?? "",
          });
        }
      } catch (error) {
  ...
  // 5. EFFECT: Đổ dữ liệu cấu hình mặc định vào Form khi fetch xong
  useEffect(() => {
    if (!defaultSetting) return;
    if (currentProduct) return; // Nếu đang edit thì dùng data sản phẩm, không dùng default setting

    setValue("vat", defaultSetting.productVat);
    setValue("unitId", defaultSetting.productUnitId);
    setValue("manufacturer", defaultSetting.defaultSupplierName);
  }, [defaultSetting, currentProduct, setValue]);

  // 6. EFFECT: Đồng bộ dữ liệu khi sản phẩm thay đổi (chuyển đổi giữa các sản phẩm cần sửa)
  useEffect(() => {
    if (currentProduct) {
      reset({
        ...defaultValues,
  ...
  // 7. EFFECT: Quản lý việc hiển thị Danh mục được chọn trên Autocomplete
  useEffect(() => {
    if (categories && categories.length > 0) {
      const cat = categories.find((c) => c.id === watch('categoryId')) || null;
      setSelectedCategory(cat);
    } else {
      setSelectedCategory(null);
    }
  ...
  // 8. HÀM SUBMIT FORM (Sạch sẽ, không chứa Hook)
  const onSubmit = handleSubmit(async (data) => {
    try {
      let imagePayload: string | null = null;
      ...
      // Refresh lại danh sách sản phẩm bằng SWR
      mutate(
        (k) => typeof k === "string" && k.startsWith("/api/v1/products"),
        undefined,
        { revalidate: true }
      ...
      if (selectedId) {
        mutate(endpoints.product.details(selectedId));
      }
    ...
    <Stack spacing={3} sx={{ p: 3 }}>
      <Stack direction={{ xs: 'column', md: 'row' }} spacing={3}>
        <Stack spacing={2} sx={{ flex: 1 }}>
          ...
          <Stack direction={{ xs: 'column', md: 'row' }} spacing={2}>
            <Field.Autocomplete
              ...
              onChange={(_, newValue) => {
                ...
                if (newValue?.vat !== undefined && newValue?.vat !== null) {
                  setValue("vat", newValue.vat);
                }
              }}
              noOptionsText="Không có dữ liệu"
          ...
          <Stack direction={{ xs: 'column', md: 'row' }} spacing={2}>
            <Field.VNCurrencyInput label='Giá nhập' name="purchasePrice" required />
            <Field.VNCurrencyInput label='Giá bán' name="price" required />
          </Stack>
          ...
          <Stack spacing={1.5}>
            ...
            {!currentProduct && (
              <Field.Select label='Thư mục tải lên' name="Folder" sx={{ display: 'none' }}>
                ...
                <MenuItem value={'HoaDon'}>Hóa đơn</MenuItem>
                <MenuItem value={'XuatKho'}>Xuất kho</MenuItem>
                <MenuItem value={'SanPham'}>Sản phẩm</MenuItem>
                <MenuItem value={'Receipt'}>Thu chi</MenuItem>
              </Field.Select>
            )}
            <Field.Upload
              thumbnail
  ...
  const renderActions = () => (
    <Box sx={{ width: '100%' }}>
      <Stack direction="row" spacing={2} width="100%" minHeight={40}>
        ...
        <Button type="submit" variant="contained" sx={{ ml: 1 }} loading={isSubmitting} fullWidth>
          {!currentProduct ? 'Tạo mới' : 'Lưu thay đổi'}
        </Button>
      </Stack>
    </Box>
