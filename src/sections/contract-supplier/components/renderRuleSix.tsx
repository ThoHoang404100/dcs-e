...
import { IContractSupplyForDetail, ResponseContractSupplier } from "src/types/contractSupplier";
...
export const renderRules = ({
    ...
    const totalPrice = currentContract?.data?.items.reduce((sum, q) => sum + q.price * q.quantity, 0) ?? 0;

    const totalVat = currentContract?.data.items?.reduce((sum, q) => {
        const lineTotal = q.price * q.quantity;
        return sum + (lineTotal * q.vat) / 100;
    }, 0) ?? 0;
    ...
    return (
        ...
        >
            <View style={styles.table}>
                ...
                <View>
                    {currentContract?.data.items?.map((p, index) => {
